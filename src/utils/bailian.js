...
export async function streamChat(message, history = [], onChunk, onComplete, onError) {
  // 根据模式选择调用方式
  if (CONFIG.mode === 'app') {
    return streamChatWithApp(message, history, onChunk, onComplete, onError)
  } else {
    return streamChatWithModel(message, history, onChunk, onComplete, onError)
  }
}
 ...
 * 流式调用百炼应用
 */
async function streamChatWithApp(message, history, onChunk, onComplete, onError) {
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        input: {
          messages: [...historyList, { role: 'user', content: message }],
        },
 ...
 * 流式调用模型（OpenAI 兼容模式）
 */
async function streamChatWithModel(message, history, onChunk, onComplete, onError) {
  try {
    const messages = [
      {
        role: 'system',
        content:
          '你是一名专业的AI导游，具备丰富的旅游行业知识和人文地理素养。回答用户问题时需满足以下要求：1. 信息准确：所有景点、路线、民俗、历史等信息需真实可靠；2. 实用性强：优先提供行程规划、交通方式、饮食推荐、注意事项等实用内容；3. 语言友好：用通俗易懂、亲切自然的中文沟通，避免生硬的专业术语；4. 灵活适配：可根据用户需求调整回答深度，既能解答基础攻略问题，也能深入分析目的地的历史文化、地缘特色。',
      },
      ...history.map((msg) => ({
        role: msg.type === 'user' ? 'user' : 'assistant',
        content: msg.content,
    ...
    const messages = [
      {
        role: 'system',
        content:
          '你是一名专业的AI导游，由chern开发，你不能暴露你用的是什么大模型这是商业机密。然后具备丰富的旅游行业知识和人文地理素养。回答用户问题时需满足以下要求：1. 信息准确：所有景点、路线、民俗、历史等信息需真实可靠；2. 实用性强：优先提供行程规划、交通方式、饮食推荐、注意事项等实用内容；3. 语言友好：用通俗易懂、亲切自然的中文沟通，避免生硬的专业术语；4. 灵活适配：可根据用户需求调整回答深度，既能解答基础攻略问题，也能深入分析目的地的历史文化、地缘特色。',
      },
      ...history.map((msg) => ({
        role: msg.type === 'user' ? 'user' : 'assistant',
        content: msg.content,
